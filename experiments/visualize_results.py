...
def plot_primary_metrics(metrics_data: dict[str, Any], output_dir: Path):
    ...
    # Extract metrics
    inference_depths = [
        methods_data[m]["primary_metrics"]["mean_inference_depth"] for m in methods
    ]
    contradiction_rates = [
        methods_data[m]["primary_metrics"]["contradiction_rate_percent"]
        for m in methods
    ]
    entailment_accs = [
        methods_data[m]["primary_metrics"]["entailment_accuracy"] for m in methods
    ]
    ...
    axes[0].bar(
        methods, inference_depths, color=colors, edgecolor="black", linewidth=1.5
    )
    ...
    axes[1].bar(
        methods, contradiction_rates, color=colors, edgecolor="black", linewidth=1.5
    )
    axes[1].set_ylabel("Contradiction Rate (%)", fontsize=12, fontweight="bold")
    axes[1].set_title(
        "(b) Contradiction Detection Rate", fontsize=13, fontweight="bold"
    )
    ...
    bars = axes[2].bar(
        methods, entailment_accs, color=colors, edgecolor="black", linewidth=1.5
    )
...
def plot_improvements(metrics_data: dict[str, Any], output_dir: Path):
    ...
    entailment_improvements = [
        comparisons[b]["entailment_accuracy"]["improvement_pct"] for b in baselines
    ]
...
def plot_per_domain_breakdown(metrics_data: dict[str, Any], output_dir: Path):
    ...
    axes[1].bar(
        domains, contradictions, color="#F4A261", edgecolor="black", linewidth=1.5
    )
    axes[1].set_ylabel("Contradiction Rate (%)", fontsize=12, fontweight="bold")
    axes[1].set_title(
        "(b) Contradiction Detection by Domain", fontsize=13, fontweight="bold"
    )
    axes[1].tick_params(axis="x", rotation=45)
    axes[1].grid(axis="y", alpha=0.3, linestyle="--")
    ...
    bars = axes[2].bar(
        domains, accuracies, color="#2A9D8F", edgecolor="black", linewidth=1.5
    )
    axes[2].set_ylabel("Entailment Accuracy", fontsize=12, fontweight="bold")
    axes[2].set_title(
        "(c) Entailment Accuracy by Domain", fontsize=13, fontweight="bold"
    )
    axes[2].tick_params(axis="x", rotation=45)
    axes[2].grid(axis="y", alpha=0.3, linestyle="--")
    axes[2].set_ylim(0, 1.0)
...
def plot_semantic_invariance(metrics_data: dict[str, Any], output_dir: Path):
    ...
    ax.set_title(
        "Semantic Invariance Across Prompt Perturbations",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_ylim(0, 1.0)
...
def create_summary_figure(metrics_data: dict[str, Any], output_dir: Path):
    ...
    ax.set_title(
        "CAF vs Baselines: All Metrics (Normalized)", fontsize=14, fontweight="bold"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Visualize CAF experiment results")
    parser.add_argument(
        "metrics_json", nargs="?", help="Path to experiment metrics JSON file"
    )
    parser.add_argument(
        "--output-dir",
